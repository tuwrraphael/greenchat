...
export class InitializationActionCreator {
    ...
    async initializeApplication() {
        let appendOnlyLogCreated = await this.localAppendOnlyLogService.appendOnlyLogCreated();
        if (!appendOnlyLogCreated) {
            this.routingActionCreator.navigateFirstTimeInit();
            await this.localAppendOnlyLogService.create();
            this.routingActionCreator.navigateHome();
        } else {
            let log = await this.localAppendOnlyLogService.get();
